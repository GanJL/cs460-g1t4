...
const HourlyChart = ({type}) => {
  ...
  const options = type !== 'R' ? {
    ...
    plugins: {
      title: {
        display: true,
        text: type == 'MS' ? 'Moisture Level vs Sunlight Level (Hourly)' : 'Temperature Level vs Humidity Level (Hourly)' ,
      },
    },
    scales: {
      x: {
  ...
  useEffect(() => {
    ...
    setInterval(getData, 1 * 60 * 15 * 1000);
  }, [type])
